...
export function migrateSave(save) {
    ...
    // 1.x 直接把 player 对象存进 localStorage（无 {saveVersion, player} 包装），
    // 这里自动识别并包装，保证老玩家的手动存档/自动存档都能被迁移
    if (!save.player && (save.name || save.cultivation || save.attributes)) {
        save = { saveVersion: 1, player: save };
    }
